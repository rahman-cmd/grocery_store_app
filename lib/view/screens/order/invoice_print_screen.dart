...
class _InVoicePrintScreenState extends State<InVoicePrintScreen> {
  ...
  Future _printReceipt({bool? isPrescriptionOrder}) async {
    ...
    for (int i = 0; i < widget.orderDetails!.length; i++) {
      bytes += generator.row([
        ...
        PosColumn(
          text: _priceDecimal(widget.orderDetails![i].price!),
          width: 3,
          styles: const PosStyles(align: PosAlign.right),
        ),
      ]);
    ...
    bytes += generator.row([
      ...
      PosColumn(
        text: _priceDecimal(itemsPrice + addOns),
        width: 6,
        styles: const PosStyles(
          align: PosAlign.right,
        ),
  ...
  Widget build(BuildContext context) {
    return _searchingMode
        ...
        : InvoiceDialog(
            ...
            onPrint: () =>
                _printReceipt(isPrescriptionOrder: widget.isPrescriptionOrder),
